...
metadata:
  ...
  source-command: .claude/commands/speckit.clarify.md
  mode: speckit.clarify
---

# speckit-clarify

Invoke this Spec Kit workflow from Codex as $speckit-clarify. If the user writes the older slash-command form, treat it as this skill's invocation and use the text after the command as the user input.
...
4. Generate (internally) a prioritized queue of candidate clarification questions (maximum 5). Do NOT output them all at once. Apply these constraints:
    ...
    - Each question must be answerable with EITHER:
       - A short multiple?hoice selection (2?? distinct, mutually exclusive options), OR
       - A one-word / short?hrase answer (explicitly constrain: "Answer in <=5 words").
...
5. Sequential questioning loop (interactive):
    ...
    - For short?nswer style (no meaningful discrete options):
